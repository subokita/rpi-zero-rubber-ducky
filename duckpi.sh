...
while IFS='' read -r line || [[ -n "$line" ]]; do
	...
	then
		...
		for ((  i=0; i<${#info}; i++  )); do
			...
			if [ "$kbcode" != "" ]
			then
				echo "$kbcode" | /home/pi/rpi-zero-rubber-ducky/hid-gadget-test $kb > /dev/null
			fi
		done
	elif [ "$cmd" == "ENTER" ]
	then
		last_cmd="enter"
		echo "$last_cmd" | /home/pi/rpi-zero-rubber-ducky/hid-gadget-test $kb > /dev/null

	elif [ "$cmd" == "DELAY" ]
	then
		last_cmd="UNS"
		((info = info*1000))
		/home/pi/rpi-zero-rubber-ducky/usleep $info
	...
	then
		...
		then
			...
			for ((  i=0; i<$info; i++  )); do
				...
				/home/pi/rpi-zero-rubber-ducky/usleep $defdelay
			done
		fi
